...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    for gorelka in hub.gorelka:
        ...
        new_devices.append(GorelkaNightModeEnableSwitch(gorelka))
    if new_devices:
...
class GorelkaNightModeEnableSwitch(SwitchEntity):
    """Representation of an Abode switch."""

    def __init__(self, gorelka):
        """Initialize the sensor."""
        self._gorelka = gorelka
        self._attr_unique_id = f"{self._gorelka.gorelka_id}_night"

        # The name of the entity
        self._attr_name = f"{self._gorelka.name} Night"
    ...
    def turn_on(self, **kwargs: Any) -> None:
        """Turn on the device."""
        self._gorelka.switch_night_mode_on()

    def turn_off(self, **kwargs: Any) -> None:
        """Turn off the device."""
        self._gorelka.switch_night_mode_off()

    @property
    def is_on(self) -> bool:
        """Return true if device is on."""
        return cast(bool, self._gorelka.is_night_mode)
